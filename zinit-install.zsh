...
.zinit-get-package() {
    ...
    if (( pos )) {
        for key_ value ( "${(@Q)${(@z)Strings[3/$(( (pos + 1) / 2 ))]}}" ) {
            (( ${+ZINIT_ICE[$key_]} )) && [[ ${ZINIT_ICE[$key_]} != +* ]] && continue
            ZINIT_ICE[$key_]=$value${ZINIT_ICE[$key_]#+}
        }
...
.zinit-download-file-stdout() {
    ...
    if (( restart )) {
        ...
        if (( ${+commands[curl]} )); then
            command curl ${progress:+-s} ${${progress:---progress-bar}:#1} -fSL "$url" || return 1
        ...
        if type curl 2>/dev/null 1>&2; then
            command curl ${progress:+-s} -fSL ${${progress:---progress-bar}:#1} "$url" || return 1
